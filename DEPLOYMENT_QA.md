...
# 4. 构建（必须！index.html是Vite入口文件）
npm run build

# 5. 删除旧的构建产物（只删除这些，不要删源代码！）
rm -f index.html
rm -rf assets
...
### ⚠️ 关键警告：绝对不要删除index.html后再构建！
```bash
# ❌ 致命错误：删除源代码后无法构建
rm -f index.html       # 删除源代码
npm run build          # 失败！找不到入口文件

# ✅ 正确做法：先构建，再删除构建产物
npm run build          # 先构建
rm -f index.html      # 再删除旧的构建产物
```
...
使用：
```bash
bash /var/www/vpnspan/deploy.sh
```
